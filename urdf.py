# Copyright (c) 2025, Micro-Intelligence, Inc. All rights reserved.
# Unauthorized copying of this file, via any medium is strictly prohibited
# Proprietary and confidential
# Author: Daoming Chen (Tim) <daoming.chen@micro-i.com.cn> 2025-05

# modified from https://github.com/mmatl/urdfpy
...
class ConvexHull(Mesh):
    """A convex hull mesh object, typically a child of <geometry>.
    Parses like a Mesh but corresponds to the <convex_hull> tag.
    """
...
class Convex(Mesh):
    """A convex mesh object, typically a child of <geometry>.
    Parses like a Mesh but corresponds to the <convex> tag.
...
class Geometry(URDFType):
    ...
    mesh : :class:`.Mesh`
        Mesh geometry.
    convex_hull : :class:`.Mesh`, optional
        Convex hull geometry.
    convex : :class:`.Mesh`, optional
        Convex geometry.
    """
    ...
    _ELEMENTS = {
        ...
        "convex_hull": (ConvexHull, False, False),
        "convex": (Convex, False, True),
    }
    _TAG = "geometry"
    ...
    def convex_hull(self):
        """:class:`.Mesh` : Convex hull geometry."""
        ...
        if value is None:
            ...
            for item in value:
                if not isinstance(item, Mesh):
                    raise TypeError("All items in convex list must be Mesh objects")
            self._convex = value
        else:
            raise TypeError(
                "Expected Mesh object, list of Mesh objects, or None for convex"
            )
    ...
    def copy(self, prefix="", scale=None):
        ...
        v = Geometry(
            ...
            convex_hull=(
                self.convex_hull.copy(prefix=prefix, scale=scale)
                if self.convex_hull
                else None
            ),
            convex=(
                [c.copy(prefix=prefix, scale=scale) for c in self.convex]
                if self.convex
                else []
            ),
        )
...
class Joint(URDFType):
    ...
    def _rotation_matrices_gpu(self, angles, axis, device):
        """Compute rotation matrices from angle/axis representations on GPU.

        Parameters
        ----------
        angles : torch.Tensor
            The angles tensor of shape (n,).
        axis : torch.Tensor or np.ndarray
            The axis tensor of shape (3,).
        device : torch.device
        ...
        -------
        rots : torch.Tensor
            The rotation matrices tensor of shape (n,4,4)
        """
        ...
        M = torch.eye(4, dtype=torch.float32, device=device).unsqueeze(0).repeat(n, 1, 1)
        M[:, 0, 0] = cosa
        M[:, 1, 1] = cosa
        M[:, 2, 2] = cosa
        
        # Outer product: axis * axis^T
        axis_outer = torch.outer(axis, axis).unsqueeze(0).repeat(n, 1, 1)
    ...
    def get_child_poses_gpu(self, cfg, n_cfgs, device):
        """GPU version of get_child_poses using PyTorch tensors.

        Parameters
        ----------
        cfg : torch.Tensor or None
            The configuration values for this joint.
        n_cfgs : int
        ...
        -------
        poses : torch.Tensor
            The poses of the child relative to the parent, shape (n_cfgs, 4, 4).
        """
        # Convert origin to torch tensor if needed
        ...
        if cfg is None:
            return origin.unsqueeze(0).repeat(n_cfgs, 1, 1)
        elif self.joint_type == "fixed":
            return origin.unsqueeze(0).repeat(n_cfgs, 1, 1)
        elif self.joint_type in ["revolute", "continuous"]:
            if cfg is None:
                cfg = torch.zeros(n_cfgs, dtype=torch.float32, device=device)
            elif isinstance(cfg, np.ndarray):
                cfg = torch.tensor(cfg, dtype=torch.float32, device=device)
            rotations = self._rotation_matrices_gpu(cfg, self.axis, device)
            return torch.matmul(origin.unsqueeze(0), rotations)
        elif self.joint_type == "prismatic":
            if cfg is None:
                cfg = torch.zeros(n_cfgs, dtype=torch.float32, device=device)
            elif isinstance(cfg, np.ndarray):
            ...
            translation = torch.eye(4, dtype=torch.float32, device=device).unsqueeze(0).repeat(n_cfgs, 1, 1)
            if isinstance(self.axis, np.ndarray):
                axis = torch.tensor(self.axis, dtype=torch.float32, device=device)
            else:
                axis = self.axis
            translation[:, :3, 3] = axis * cfg.unsqueeze(-1)
            return torch.matmul(origin.unsqueeze(0), translation)
        elif self.joint_type == "planar":
            raise NotImplementedError("Planar joints not implemented for GPU version")
        elif self.joint_type == "floating":
            raise NotImplementedError("Floating joints not implemented for GPU version")
        else:
            raise ValueError("Invalid configuration")
...
class URDF(URDFType):
    ...
    def link_fk_batch_gpu(self, cfgs, device, link=None, links=None, use_names=False):
        """GPU version of link_fk_batch using PyTorch tensors.

        Parameters
        ----------
        cfgs : torch.Tensor or np.ndarray
            Configuration array of shape (n_cfgs, n_joints).
        device : torch.device
            The device to perform computation on.
        link : str or :class:`.Link`
            A single link or link name to return a pose for.
        ...
        Returns
        -------
        fk : dict or torch.Tensor
            A map from links to (n_cfgs,4,4) tensor of homogenous transform matrices that
            position the links relative to the base link's frame, or a single
            (n_cfgs,4,4) tensor if ``link`` is specified.
        """
        joint_cfgs, n_cfgs = self._process_cfgs_gpu(cfgs, device)
        ...
        # Compute FK mapping each link to a tensor of matrices, one matrix per cfg
        fk = OrderedDict()
        eye_4x4 = torch.eye(4, dtype=torch.float32, device=device)
        
        for lnk in self._reverse_topo:
            if lnk not in link_set:
                continue
            poses = eye_4x4.unsqueeze(0).repeat(n_cfgs, 1, 1)
            path = self._paths_to_base[lnk]
            for i in range(len(path) - 1):
                child = path[i]
                parent = path[i + 1]
                ...
                child_poses = joint.get_child_poses_gpu(cfg_vals, n_cfgs, device)
                poses = torch.matmul(child_poses, poses)
    ...
    def _process_cfgs_gpu(self, cfgs, device):
        """GPU version of _process_cfgs using PyTorch tensors.
        ...
        -------
        joint_cfg : dict
            Dictionary mapping joints to configuration tensors.
        n_cfgs : int
            Number of configurations.
        """
        if isinstance(cfgs, np.ndarray):
            cfgs = torch.tensor(cfgs, dtype=torch.float32, device=device)
        elif isinstance(cfgs, torch.Tensor):
            cfgs = cfgs.to(device=device, dtype=torch.float32)
        else:
            raise ValueError("GPU version only supports tensor inputs")
